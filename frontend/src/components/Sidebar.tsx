...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside className="app-sidebar">
      <div>
        {/* Account Mode Segmented Toggle (or Admin Badge if Admin) */}
        {isAdmin ? (
          <div
            style={{
              padding: '8px 12px',
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: '#f3e8ff' }}>{t.adminBadge}</span>
              <span style={{ fontSize: 10, color: '#d8b4fe' }}>{user.name}</span>
            </div>
          </div>
        ) : (
        ...
        <div className="sidebar-nav">
          ...
          >
            ...
            {dealsCount > 0 && (
              <span className="nav-badge" style={{ backgroundColor: 'rgba(168, 185, 129, 0.12)', color: '#10b981', borderColor: 'rgba(16, 185, 129, 0.3)' }}>
                {dealsCount}
              </span>
            )}
          </button>
          ...
          {/* Admin & Agent Harness Tab - ONLY for Admins */}
          {isAdmin && (
            <button
