...
function generateBidsForObligation(obl: Obligation): MarketplaceOpportunity {
  const isTry = obl.amount.currency === 'TRY'
  ...
  if (titleLower.includes('kasko') || titleLower.includes('sigorta')) {
    ...
    bids = [
      ...
      },
      {
        ...
        savingsRate: 39.0,
        terms: 'Taahhütsüz, kotasız, simetrik hız ve ücretsız modem kullanımı.',
        contractUrl: 'https://turk.net',
        status: 'PENDING',
        riskScore: 0.05,
        riskNotes: 'Taahhütsüz esnek iptal seçeneği.',
        createdAt: new Date().toISOString(),
      },
    ]
  } else if (titleLower.includes('aws') || titleLower.includes('cloud') || titleLower.includes('hosting') || titleLower.includes('sunucu')) {
    ...
    bids = [
      {
        ...
        price: { amount: 1176, currency: obl.amount.currency },
        savingsAmount: { amount: 1274, currency: obl.amount.currency },
        savingsRate: 52.0,
      ...
      },
      {
        ...
        price: { amount: 1450, currency: obl.amount.currency },
        savingsAmount: { amount: 1000, currency: obl.amount.currency },
        savingsRate: 40.8,
    ...
    bids = [
      {
        ...
        price: { amount: Math.round(obl.amount.amount * 0.55), currency: obl.amount.currency },
        savingsAmount: { amount: Math.round(obl.amount.amount * 0.45), currency: obl.amount.currency },
        savingsRate: 45.0,
  ...
  async approveObligation(id: string, executeMCP = true): Promise<any> {
    try {
      ...
      state.obligations = state.obligations.map((o) =>
        o.id === id ? { ...o, status: 'APPROVED', updatedAt: new Date().toISOString() } : o
      )
    ...
    state.opportunities.forEach((opp) => {
      opp.bids?.forEach((bid) => {
        if (bid.status === 'ACCEPTED') {
          ...
          totalSavings += bid.savingsAmount.amount
          dealsCount++
        }
      })
    })
